...
def parse_xml_library(xml_path: str) -> Tuple[Optional[Dict], Optional[Dict], float, Optional[str]]:
    ...
    Returns:
        Tuple of (artist_data, song_data, total_time_ms, error_message)
        If error occurs, first three elements are None/0 and error_message is set.
    """
    try:
    ...
    for song in lib.songs.values():
        # Skip songs without play count or length
        if not song.play_count or not song.length:
            continue
        ...
        # Get artist names with fallback
        artist = song.album_artist if song.album_artist else (song.artist if song.artist else "Unknown")
        specific_artist = song.artist if song.artist else "Unknown"
        album = song.album if song.album else "Unknown"
        song_name = song.name if song.name else "Unknown"
...
def calculate_stats(old_res: Tuple, new_res: Tuple, unit: str = UNIT_HOURS) -> Tuple:
    ...
    # Calculate artist/album differences
...
class TunesBackApp:
    def __init__(self, page: ft.Page):
        ...
        self.cached_sorted = {}  # Cache for sorted DataFrames


        # Store raw parse results for unit conversion
    ...
    def _init_controls(self):
        ...
        self.txt_single_file_warning = ft.Text(
            "Only 1 snapshot found. Comparison disabled.",
            size=11, color=ft.Colors.AMBER_400, visible=False
        )


        self.btn_select = ft.ElevatedButton(
            "Select Folder", icon="folder", width=float("inf"), 
        ...
        self.dd_start = ft.Dropdown(
            ...
            dense=True, border="outline", disabled=True, width=Theme.TOGGLE_WIDTH,
        ...
        self.cb_compare = ft.Checkbox(
            label="Compare", value=True, disabled=True, 
            on_change=self.on_compare_changed
        )
        ...
        self.seg_unit = ft.SegmentedButton(
            selected={UNIT_HOURS}, 
            segments=[
                ft.Segment(value=UNIT_HOURS, label=ft.Text(UNIT_HOURS)),
                ft.Segment(value=UNIT_MINUTES, label=ft.Text(UNIT_MINUTES))
            ],
            disabled=True, allow_multiple_selection=False, 
            on_change=self.on_unit_changed, width=Theme.TOGGLE_WIDTH
        )
        self.seg_sort = ft.SegmentedButton(
            ...
            disabled=True, allow_multiple_selection=False, 
            on_change=self.update_results_ui, width=Theme.TOGGLE_WIDTH
        )


        # Action Buttons
        self.btn_run = ft.ElevatedButton(
            "Generate Recap", icon="bar_chart",
        ...
        self.btn_cancel = ft.ElevatedButton(
            "Cancel", icon="close", color="error",
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8), padding=20),
            expand=True, visible=False, on_click=self.cancel_analysis_handler
        )
        self.btn_reset = ft.IconButton(
            icon="refresh", tooltip="Reset View", 
            disabled=True, on_click=self.reset_view
        )
        self.btn_theme = ft.IconButton(
            icon="light_mode", icon_size=18, 
            tooltip="Toggle Theme", on_click=self.toggle_theme
        )
        ...
        self.tabs_main = ft.Tabs(
            ...
            tabs=[
                ft.Tab(text="Top Artists", icon="mic"), 
                ft.Tab(text="Top Albums", icon="album"),
        ...
        self.card_vals = [
            ft.Text("-", size=16, weight="bold", overflow="ellipsis", text_align="center", no_wrap=True)
            for _ in range(3)
        ]
        self.card_subs = [
            ft.Text("-", size=11, color=Theme.SUBTEXT, text_align="center") 
            for _ in range(3)
        ]


        self.txt_app_title = ft.Text(
            "TunesBack", weight="bold", size=14, color=Theme.SUBTEXT, 
            opacity=0, animate_opacity=300
        )


        # Loading status text
        self.txt_loading_status = ft.Text(
            "Crunching numbers...", 
            size=16, weight="bold", color=Theme.SUBTEXT
        )
        ...
        self.modal_container = ft.Container(
            content=ft.Container(
                content=ft.Column([
                    ...
                    ft.Row([
                        ft.TextButton("OK", on_click=lambda e: self.toggle_modal(False))
                    ], alignment="end")
            ...
            alignment=ft.alignment.center, visible=False, expand=True
        )
    ...
    def _build_layout(self):
        ...
        self.view_welcome = ft.Container(
            content=ft.Column([
                ...
                ft.Text(
                    "Select a folder containing your library snapshots to begin.", 
        ...
        kpi_cards = ft.Row([
            create_kpi_card(
                ft.Column([
                    ft.Text("Top Artist", size=10, color=Theme.SUBTEXT), 
                    self.card_vals[0], self.card_subs[0]
                ], horizontal_alignment="center", alignment="center", spacing=2)
            ),
            create_kpi_card(
                ft.Column([
                    ft.Text("Top Album", size=10, color=Theme.SUBTEXT), 
                    self.card_vals[1], self.card_subs[1]
                ], horizontal_alignment="center", alignment="center", spacing=2)
            ),
            create_kpi_card(
                ft.Column([
                    ft.Text("Top Song", size=10, color=Theme.SUBTEXT), 
                    self.card_vals[2], self.card_subs[2]
                ], horizontal_alignment="center", alignment="center", spacing=2)
            ),
        ...
        self.view_dash = ft.Column([
            ft.Container(
                ...
                content=ft.Column([
                    ft.Container(height=10), header_row, ft.Container(height=20), 
                    self.tabs_main, ft.Container(height=10)
                ])
            ),
            ft.Container(
                padding=ft.padding.only(left=Theme.PAD_LEFT, right=20, bottom=20), 
                content=results_box, expand=True
            )
        ...
        sidebar_inner = ft.Container(padding=20, content=ft.Column([
            ft.Row([
                ft.Text("SETTINGS", size=12, weight="bold", color=Theme.SUBTEXT), 
                self.btn_theme
            ], alignment="spaceBetween"),
            ft.Divider(), 
            ...
            self.txt_path,
            self.txt_file_count,
            self.txt_single_file_warning,
            ft.Divider(), 
            ft.Text("Period", size=12, weight="bold"),
            ft.Column([
                self.dd_start,
                ft.Row([
                    self.dd_end, self.cb_compare
            ...
            ft.Column([
                ft.Text("Time Unit", size=11, color=Theme.SUBTEXT), 
                self.seg_unit,
                ft.Text("Rank By", size=11, color=Theme.SUBTEXT), 
                self.seg_sort
            ], spacing=5),
            ft.Container(height=20), 
            ft.Stack([
                ft.Row([self.btn_run, self.btn_reset], spacing=10),
                self.btn_cancel
            ])
        ], spacing=10))
        ...
        sidebar = ft.Container(
            width=Theme.SIDEBAR_WIDTH, bgcolor=Theme.SIDEBAR_BG, padding=0,
        ...
        window_header = ft.WindowDragArea(content=ft.Container(
            ...
            padding=ft.padding.only(left=Theme.PAD_LEFT, right=20, top=20, bottom=10), 
        ...
        self.main_layout = ft.Container(
            content=ft.Stack([
                ft.Row([
                    sidebar, 
                    ft.Container(
                        content=ft.Column([window_header, content_area], spacing=0), 
                        expand=True
                    )
    ...
    def on_folder_picked(self, e: ft.FilePickerResultEvent):
        ...
        if self.files:
            ...
            if len(self.files) == 1:
                self.txt_single_file_warning.visible = True
                self.cb_compare.value = False
                self.cb_compare.disabled = True
                self.dd_end.value = None
                self.dd_end.disabled = True
            else:
                # Multiple files detected - auto-select Compare
                self.txt_single_file_warning.visible = False
            ...
            # Enable controls
            for c in [self.dd_start, self.btn_run, self.sl_art, self.sl_alb, 
                      self.sl_song, self.seg_unit, self.seg_sort]:
                c.disabled = False
    ...
    def on_start_changed(self, e):
        ...
        if self.dd_end.value == self.dd_start.value:
            self.dd_end.value = None
        self.dd_end.update()


    def on_unit_changed(self, e):
        """Handle unit change and recalculate all values in real-time"""
        # Only recalculate if we have raw data
        if self.raw_res_start is None:
            return

        unit = list(self.seg_unit.selected)[0]

        # Recalculate stats with new unit
        if self.is_compare_mode and self.raw_res_end is not None:
            val_main, val_plays, df_art, df_alb, df_song = calculate_stats(
                self.raw_res_start, self.raw_res_end, unit
            )
        else:
            val_main, val_plays, df_art, df_alb, df_song = calculate_single_stats(
                self.raw_res_start, unit
            )

        # Update data frames
        self.data_frames = {"art": df_art, "alb": df_alb, "song": df_song}
        self.cached_sorted.clear()  # Clear cache since values changed

        # Update KPI values
        self.kpi_growth.value = f"{val_main:,.1f}"
        self.kpi_plays.value = f"{int(val_plays):,}"

        # Update labels and cards
    ...
    def update_kpi_units(self):
        """Update KPI card text labels based on selected unit"""
        unit = list(self.seg_unit.selected)[0]


        # Update main KPI labels
    ...
    def run_analysis(self, e):
        ...
        if not self.dd_start.value:
            return
        if self.cb_compare.value and not self.dd_end.value:
            return
    ...
    def _run_analysis_thread(self):
        """Background thread for analysis (Phase 1: Threading)"""
        try:
            # Get file references
            start_file = next(f for f in self.files if f['label'] == self.dd_start.value)
            ...
            if self.cb_compare.value:
                ...
                # Calculate single snapshot stats
            ...
            self.data_frames = {"art": df_art, "alb": df_alb, "song": df_song}
            self.cached_sorted.clear()  # Clear cache
    ...
    def update_results_ui(self, e):
        """Update the results list (Phase 1: Caching)"""
        ...
        if df.empty:
            self.list_results.controls.append(
                ft.Text("No data to display.", italic=True)
            )
        else:
            # Use cached sorted DataFrame if available
            ...
            for i, row in enumerate(df_sorted.itertuples(), 1):
                ...
                self.list_results.controls.append(
                    draw_list_item(i, lbl, sub, getattr(row, "Value"), 
                                 getattr(row, "Count"), unit, color)
                )
